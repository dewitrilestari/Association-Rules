...
def recommend_retail_cf(target_nama_produk, n=10):
    ...
    mapping_stock = dict(zip(df_master_cf['nama_produk'], df_master_cf['stock_code']))
    mapping_cat = dict(zip(df_master_cf['nama_produk'], df_master_cf['product_category']))
    
    result['Stock Code'] = result['Nama Produk Rekomendasi'].map(mapping_stock)
    ...
    return result[['Stock Code', 'Nama Produk Rekomendasi', 'Kategori Produk', 'Pearson Correlation', 'Alasan Rekomendasi']]
